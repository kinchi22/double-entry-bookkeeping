...
| Copy             | Text a person reads in the UI: JSX text, a string on an attribute not listed as markup, a string a JSX expression renders, page metadata. Lives in the message catalogue, never inline; `repo/no-inline-copy`. |
...
| Message catalogue | `apps/web/messages/{locale}.ts`: the copy for one locale, a plain object keyed in English, one namespace per component that renders copy. `en.ts` is the only one until ADR-0007 is adopted. |
...
- Copy lives in `apps/web/messages/en.ts`, never inline, enforced by the
  `repo/no-inline-copy` lint rule. Catalogue keys are English, and `en` is the
  default locale.
